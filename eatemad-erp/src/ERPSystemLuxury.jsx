...
const safeUser = (user, language) => {
  ...
  return {
    ...
    title: user?.title || (language === "ar" ? "مدير الموارد البشرية" : "HR Manager"),
    permissions: Array.isArray(user?.permissions) && user.permissions.length ? user.permissions : ["hr"],
...
function ERPSystemLuxury({
  ...
  const modules = useMemo(
    () =>
      ...
      })).filter((m) => userHasPermission(currentUser.permissions, m.permission)),
  ...
  return (
    <ThemeContext.Provider value={{ isDarkMode, currentTheme: theme, language }}>
      <div style={{ minHeight: "100vh", background: theme.bgGradient, color: theme.text, direction: language === "ar" ? "rtl" : "ltr" }}>
        {/* header */}
        <header style={{ height: 75, display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "0 1rem" : "0 2rem", background: theme.glass, borderBottom: `1px solid ${theme.border}`, position: "sticky", top: 0, zIndex: 1000 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.85rem", minWidth: 0 }}>
            <button onClick={() => setMobileSidebarOpen((v) => !v)} style={{ display: isMobile ? "inline-flex" : "none", background: "transparent", border: "none", color: theme.accent, cursor: "pointer" }}><FiMenu size={22} /></button>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: `linear-gradient(145deg, ${Colors.gold}, ${Colors.goldDark})`, display: "grid", placeItems: "center", border: `2px solid ${Colors.goldLight}` }}>
              <img src="/eatemad-logo.png" alt="logo" style={{ width: "82%", height: "82%", objectFit: "contain" }} />
            </div>
            <div style={{ minWidth: 0 }}>
              <h1 style={{ margin: 0, fontSize: isMobile ? "1.1rem" : "1.5rem", fontWeight: 800, whiteSpace: "nowrap" }}>{language === "ar" ? "نظام الاعتماد المتكامل" : "Al Eatemad ERP"}</h1>
              {!isMobile && <p style={{ margin: 0, color: theme.textMuted, fontSize: "0.8rem" }}>{language === "ar" ? "إدارة الموارد المؤسسية" : "Enterprise Resource Planning"}</p>}
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
            {!isTablet && (
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 24, padding: "0.5rem 0.9rem", minWidth: 250 }}>
                <FiSearch size={16} color={theme.accent} />
                <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={language === "ar" ? "البحث في النظام..." : "Search system..."} style={{ width: "100%", background: "transparent", border: "none", outline: "none", color: theme.text, fontFamily: "inherit" }} />
                {searchQuery && <button onClick={() => setSearchQuery("")} style={{ background: "transparent", border: "none", color: theme.textMuted, cursor: "pointer" }}><FiX size={15} /></button>}
              </div>
            )}
        ...
        <div style={{ display: "flex", minHeight: "calc(100vh - 75px)", position: "relative" }}>
          {isMobile && mobileSidebarOpen && (
            <div
              style={{ position: "fixed", top: 75, left: 0, right: 0, bottom: 0, background: "rgba(10,8,6,0.8)", zIndex: 998 }}
          ...
          <aside
            style={{
              ...
              borderLeft: language === "en" ? `1px solid ${theme.border}` : "none",
              ...
              transform: isMobile ? (mobileSidebarOpen ? "translateX(0)" : `translateX(${language === "ar" ? "110%" : "-110%"})`) : "none",
            }}
          >
            {!isMobile && (
              <button
                onClick={() => setSidebarExpanded((v) => !v)}
                style={{ position: "absolute", top: 24, [language === "ar" ? "left" : "right"]: -15, width: 30, height: 30, borderRadius: "50%", border: "none", background: `linear-gradient(135deg, ${theme.accent}, ${theme.accentLight})`, color: isDarkMode ? Colors.bgDark : "#fff", display: "grid", placeItems: "center", cursor: "pointer" }}
              >
                {sidebarExpanded ? (language === "ar" ? <FiChevronRight size={14} /> : <FiChevronDown size={14} />) : language === "ar" ? <FiChevronDown size={14} /> : <FiChevronRight size={14} />}
              </button>
            )}
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 13,
                      marginBottom: "0.6rem",
                      padding: sidebarExpanded || isMobile ? "0.95rem 1rem" : "0.95rem",
                  ...
                  >
                    <Icon size={20} />
                    {(sidebarExpanded || isMobile) && <span style={{ fontSize: "0.92rem", fontWeight: active ? 700 : 600 }}>{m.label}</span>}
                  </button>
                );
              })}
            </nav>
            ...
            {(sidebarExpanded || isMobile) && (
              <div style={{ padding: "0 0.95rem", marginTop: "auto" }}>
                <button style={{ width: "100%", padding: "0.85rem", borderRadius: 10, border: `1px solid ${theme.border}`, background: "transparent", color: theme.textMuted, marginBottom: "0.55rem", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", cursor: "pointer" }}>
                  <FiSettings size={17} />
                  <span>{language === "ar" ? "الإعدادات" : "Settings"}</span>
                </button>
                <button onClick={() => onLogout?.()} style={{ width: "100%", padding: "0.85rem", borderRadius: 10, border: "none", background: `linear-gradient(135deg, ${Colors.red}, ${Colors.darkRed})`, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", cursor: "pointer", fontWeight: 700 }}>
                  <FiLogOut size={17} />
...
function HRSubModule({ module, theme, language, isCompact, moduleData }) {
  ...
  const config = {
    ...
    attendance: { icon: FiCheckCircle, title: t("الحضور والانصراف", "Attendance"), sub: t("تتبع حضور الموظفين", "Track attendance") },
    leaves: { icon: FiCalendar, title: t("إدارة الإجازات", "Leave Management"), sub: t("طلبات الإجازات", "Leave requests") },
    payroll: { icon: FiCreditCard, title: t("إدارة الرواتب", "Payroll"), sub: t("تشغيل الرواتب", "Process payroll") },
    ...
    <div>
      ...
      <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 14, padding: isCompact ? "1rem" : "1.3rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.7rem" }}>
          ...
          <button style={{ border: "none", borderRadius: 8, background: `linear-gradient(135deg, ${theme.accent}, ${theme.accentLight})`, color: "#111", padding: "0.45rem 0.75rem", display: "flex", alignItems: "center", gap: "0.4rem", cursor: "pointer", fontWeight: 700 }}>
